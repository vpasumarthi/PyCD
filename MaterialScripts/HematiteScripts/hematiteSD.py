...
def hematiteSD(systemSize, pbc, Temp, cutE, speciesCount, tFinal, nTraj, stepInterval, kmcStepCountPrecision, 
    ...
    if not os.path.exists(workDirPath):
        ...
        sdAnalysisData = hematiteAnalysis.computeSD(workDirPath, report)
        sdData = sdAnalysisData.sdData
        speciesTypes = sdAnalysisData.speciesTypes
        fileName = sdAnalysisData.fileName
        hematiteAnalysis.generateSDPlot(sdData, speciesTypes, fileName, workDirPath)