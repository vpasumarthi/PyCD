...
class System(object):
    ...
    def get_precise_step_change_data(self, charge_list_prod, alpha, lower_bound,
                                     ...
                                     num_data_points, dst_path, sub_prefix_list):
    ...
    def get_cutoff_parameters(self, tau_ratio, dst_path, prefix_list):
        ...
        if self.r_cut == 'simulation_cell':
            ...
            if isinstance(self.k_cut, list):
                ...
                fourier_space_parameters = self.minimize_fourier_space_cutoff_error(charge_list_einsum, volume_derived_length, fourier_space_parameters, x_fourier_initial_guess)
                k_cut_estimate = fourier_space_parameters['k_cut']
                print(f'Starting with an estimate for k_cut={k_cut_estimate * constants.ANG2BOHR:.3e}')
            ...
            print(f'Preliminary convergence in absolute value of Fourier-space energy achieved within k_cut={k_cut_upper * constants.ANG2BOHR:.3e}\n')
            ...
            if np.isreal(self.alpha) & np.isreal(self.r_cut):
                ...
                if self.check_for_r_cut_convergence(charge_list_prod, alpha, r_cut_max, threshold_fractional_r_cut, upper_bound):
                    ...
                    print(f'This code doesn\'t support user-specified alpha={alpha * constants.ANG2BOHR:.3e} as r_cut is over L/2. Please re-run at a different alpha value.')
    
                    prefix_list.append(f'alpha: {alpha * constants.ANG2BOHR:.3e} / angstrom ({choice_parameters["r_cut"]})\n')
                    prefix_list.append(f'This code doesn\'t support user-specified alpha={alpha * constants.ANG2BOHR:.3e} as r_cut is over L/2. Please re-run at a different alpha value.\n\n')
    ...
    def get_precomputed_array(self, dst_path, compute_energy_contributions):
        ...
        if compute_energy_contributions:
            ...
            prefix_list.append(f'Energy contribution from Real space: {real_space_energy/ constants.EV2HARTREE:.3e} eV\n')

            fourier_space_energy = np.sum(np.multiply(charge_list_prod, precomputed_array_fourier))
            prefix_list.append(f'Energy contribution from Fourier-space: {fourier_space_energy / constants.EV2HARTREE:.3e} eV\n')
